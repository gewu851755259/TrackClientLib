...
public class TrackNameUtils {
    ...
    public static String getTrackName(Context context, long trackId, long startTime, Location location) {
        ...
        } else if (trackName.equals(context.getString(R.string.settings_recording_track_name_number_value))) {
            if (trackId == -1L) {
                return null;
            }
            return context.getString(R.string.track_name_format, trackId);
        } else {
            // trackName equals settings_recording_track_name_location_value or
            // any
            // value
            if (location != null) {
    ...
    private static String getReverseGeoCoding(Context context, Location location) {
        if (location == null /*
                              * ||
                              * !ApiAdapterFactory.getApiAdapter().isGeoCoderPresent
                              * ()
                              */) {
            return null;
        }
