...
import com.mapscloud.track.android.client.ControlTower;
...
public class TrackRecordingServiceConnectionUtils {
    ...
    public static boolean isRecordingServiceRunning(Context context) {
        ...
        for (RunningServiceInfo serviceInfo : services) {
            ...
            if (Constant.TRACKS_SERVICE_CALSS_NAME.equals(serviceName)) {
                return true;
            }
        }
        return false;
            ...
            try {
                if (showEditor) {
                    ...
                    if (recordingTrackId != PreferencesUtils.RECORDING_TRACK_ID_DEFAULT) {
                        // edit finished track
                        /*
                         * Intent intent = IntentUtils.newIntent(context,
     ...
     * Adds a marker.
     */
    public static long addMarker(Context context, TrackRecordingServiceConnection trackRecordingServiceConnection,
        ...
        if (trackRecordingService == null) {
            ...
            try {
                id = trackRecordingService.insertWaypoint(waypointCreationRequest);
                if (id != -1L) {
     ...
     * Adds a marker.
     */
    public static long addMarker(Context context, ControlTower controlTower,
                                 WaypointCreationRequest waypointCreationRequest) {
        long id = -1L;
        ITrackRecordingService trackRecordingService = controlTower.getTrackServices();
        if (trackRecordingService == null) {
            Log.d(TAG, "Unable to add marker, no track recording service");
        } else {
            try {
