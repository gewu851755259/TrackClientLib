...
/**
 * yml标记，不应该让InfoRecorder直接使用，要做封装
 */
...
public interface MyTracksProviderUtils {
     ...
     * provider.
     */
//    String AUTHORITY = "com.stmap.app.mytracks";
    String AUTHORITY = "com.yml.localtrack";

    /**
     * Clears a track. Removes waypoints and trackpoints. Only keeps the track
     * id.
     * 
     * @param trackId
     *            the track id
     */
    void clearTrack(long trackId);

    String getDefaultTitle(String currentName);
    /**
     * Creates a {@link Track} from a cursor.
     * 
     * @param cursor
     *            the cursor pointing to the track
     */
    ...
    void updateAllPointInTrack(long trackId, String name, List<Location> points) throws Exception;
    ...
    List<Track> getAllNaviTracks();
    List<Track> getTracks(String selection, String[] selectionArgs);

    List<Track> getAllTracksByKey(String key);

    List<Track> getFavoritedTracks();
    ...
    /**
     * Gets a track cursor. The caller owns the returned cursor and is
     * responsible for closing it.
     * 
     * @param selection
     *            the selection. Can be null
     * @param selectionArgs
     *            the selection arguments. Can be null
     * @param sortOrder
     *            the sort order. Can be null
     */
     ...
     * 
     * @param cursor
     *            the cursor pointing to the waypoint
     */
     ...
     *            the description generator. Can be null for waypoint marker
     */
    void deleteWaypoint(long waypointId, DescriptionGenerator descriptionGenerator);

    /**
     * Gets the first waypoint id for a track. The first waypoint is special as
     * it contains the stats for the track. Returns -1L if it doesn't exist.
     * 
     * @param trackId
     *            the track id
     */
    ...
    /**
     * Gets the next waypoint number for a type. Returns -1 if not able to get
     ...
     *            the sort order. Can be null
     * @param maxWaypoints
     *            the maximum number of waypoints to return. -1 for no limit
     */
    Cursor getWaypointCursor(String selection, String[] selectionArgs, String sortOrder, int maxWaypoints);

    /**
     * Gets a waypoint cursor for a track. The caller owns the returned cursor
     * and is responsible for closing it.
     * 
     * @param trackId
     *            the track id
     * @param minWaypointId
     *            the minimum waypoint id. -1L to ignore
     * @param maxWaypoints
     *            the maximum number of waypoints to return. -1 for no limit
     */
     ...
     * @param length
     *            the number of locations (from the beginning of the array) to
     ...
     * 
     * @param cursor
     *            the cursor pointing to the location
     */
    ...
    /**
     * Gets the last valid location for a track. Returns null if it doesn't
     * exist.
     * 
     * @param trackId
     *            the track id
     ...
     * @param trackId
     *            the track id
     * @param startTrackPointId
     *            the starting track point id. -1L to ignore
     * @param maxLocations
     *            maximum number of locations to return. -1 for no limit
     * @param descending
     *            true to sort the result in descending order (latest location
     *            first)
     */
    Cursor getTrackPointCursor(long trackId, long startTrackPointId, int maxLocations, boolean descending);

    /**
     * Creates a new read-only iterator over a given track's points. It provides
     * a lightweight way of iterating over long tracks without failing due to
     * the underlying cursor limitations. Since it's a read-only iterator,
     ...
     */
    LocationIterator getTrackPointLocationIterator(long trackId, long startTrackPointId, boolean descending,
    ...
    void updateUploadInfo(Context context, ContentValues values,
    ...
    class Factory {
        ...
        /**
         * Overrides the factory instance for testing. Don't forget to set it
         * back to the original value after testing.
         * 
         * @param factory
         *            the factory
         */
        ...
        /**
         * Creates an instance of {@link MyTracksProviderUtils}. Allows
         * subclasses to override for testing.
         * 
         * @param context
         *            the context
         */
