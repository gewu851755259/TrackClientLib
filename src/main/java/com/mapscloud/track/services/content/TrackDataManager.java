...
public class TrackDataManager {
    ...
    private final Map<TrackDataListener, EnumSet<TrackDataType>> listenerToTypesMap = new HashMap<TrackDataListener, EnumSet<TrackDataType>>();

    // Map of track data type to listeners
    private final Map<TrackDataType, Set<TrackDataListener>> typeToListenersMap = new EnumMap<TrackDataType, Set<TrackDataListener>>(
            TrackDataType.class);
     ...
     *            the track data types the listener is interested
     */
    public void registerListener(TrackDataListener listener, EnumSet<TrackDataType> trackDataTypes) {
        if (listenerToTypesMap.containsKey(listener)) {
            Log.w("track", "Tried to register a listener that is already registered. Ignore.");
            return;
        }
    ...
    public void unregisterListener(TrackDataListener listener) {
        ...
        if (removedTypes == null) {
            Log.w("track", "Tried to unregister a listener that is not registered. Ignore.");
            return;
        }
