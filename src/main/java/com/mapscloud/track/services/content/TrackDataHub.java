...
import com.mapscloud.track.services.content.TrackDataListener.LocationState;
import com.mapscloud.track.services.model.Waypoint;
import com.mapscloud.track.services.provider.MyTracksProviderUtils;
import com.mapscloud.track.services.provider.MyTracksProviderUtils.LocationIterator;
...
import static com.mapscloud.track.services.utils.Constant.MAX_DISPLAYED_WAYPOINTS_POINTS;
import static com.mapscloud.track.services.utils.Constant.MAX_LOCATION_AGE_MS;
import static com.mapscloud.track.services.utils.Constant.MAX_NETWORK_AGE_MS;
import static com.mapscloud.track.services.utils.Constant.TARGET_DISPLAYED_TRACK_POINTS;
...
public class TrackDataHub implements DataSourceListener {
    ...
    private final String commomTrackId;
    private final String commonTrackPaused;
    ...
    TrackDataHub(Context context, TrackDataManager trackDataManager, MyTracksProviderUtils myTracksProviderUtils,
                 int targetNumPoints) {
        this.context = context;
        this.appId = context.getPackageName();
        this.commomTrackId = context.getResources().getString(R.string.recording_track_id_key);
        this.commonTrackPaused = context.getResources().getString(R.string.recording_track_paused_key);
    ...
    public void loadTrack(final long trackId) {
        Log.i("data_hub", "loadTrack " + trackId);
        runInHanderThread(new Runnable() {
            @Override
            public void run() {
                ...
                Timber.i("loadTrack trackId = %d 并且赋值给selectedTrackId", trackId);
    ...
    public void registerTrackDataListener(final TrackDataListener trackDataListener,
            final EnumSet<TrackDataType> trackDataTypes) {
        runInHanderThread(new Runnable() {
            @Override
            public void run() {
    ...
    public boolean isSelectedTrackRecording() {
        Timber.i("selectedTrackId = %d, recordingTrackId = %d", selectedTrackId, recordingTrackId);
    ...
    public void notifyTrackPointsTableUpdated() {
        runInHanderThread(new Runnable() {
            @Override
            public void run() {
                Log.i("data_hub", "loadDataForListener");
...
//                }
                ...
                // yml 修改赋值recordingTrackId 和 recordingTrackPaused

                if (key == null || key.equals(appId + commomTrackId)){
                    recordingTrackId = PreferencesUtils.getLong(context, appId + commomTrackId);
                }
                if (key == null || key.equals(appId + commonTrackPaused)) {
                    recordingTrackPaused = PreferencesUtils.getBoolean(context, appId + commonTrackPaused,
                ...
                if (key == null || key.equals(PreferencesUtils.getKey(context, R.string.min_required_accuracy_key))) {
                ...
                if (key == null || key.equals(PreferencesUtils.getKey(context, R.string.min_recording_distance_key))) {
    ...
    private void loadDataForAll() {
        ...
        if (lastSeenLocation != null) {
            notifyLocationChanged(lastSeenLocation, true, trackDataManager.getListeners(TrackDataType.LOCATION));
        } else {
            notifyLocationStateChanged(trackDataManager.getListeners(TrackDataType.LOCATION));
        }
        notifyHeadingChange(trackDataManager.getListeners(TrackDataType.HEADING));
    ...
    private void loadDataForListener(TrackDataListener trackDataListener) {
        ...
        if (hasSampledIn || hasSampledOut) {
            ...
            Set<TrackDataListener> sampledOutListeners = hasSampledOut ? trackDataListeners : Collections
                    .<TrackDataListener> emptySet();
    ...
    /**
     * Notifies waypoint table update. Currently, reloads all the waypoints up
     * to {@link com.mapscloud.track.services.utils.Constant#MAX_DISPLAYED_WAYPOINTS_POINTS}. To be run in the
     * {@link #handler} thread.
     * 
     * @param trackDataListeners
     *            the track data listeners to notify
    ...
    private void notifyWaypointsTableUpdate(Set<TrackDataListener> trackDataListeners) {
        ...
        Cursor cursor = null;
        try {
            cursor = myTracksProviderUtils.getWaypointCursor(selectedTrackId, -1L, MAX_DISPLAYED_WAYPOINTS_POINTS);
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    Waypoint waypoint = myTracksProviderUtils.createWaypoint(cursor);
                    if (!LocationUtils.isValidLocation(waypoint.location)) {
                        continue;
                    }
                    for (TrackDataListener trackDataListener : trackDataListeners) {
     ...
     */
    private void notifyTrackPointsTableUpdate(boolean updateSamplingState, Set<TrackDataListener> sampledInListeners,
