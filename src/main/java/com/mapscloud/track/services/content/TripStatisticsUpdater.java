...
import com.mapscloud.track.services.tracks.DoubleBuffer;
...
public class TripStatisticsUpdater {
    ...
    private final DoubleBuffer speedBuffer = new DoubleBuffer(Constant.SPEED_SMOOTHING_FACTOR);

    // A buffer of the recent elevation readings (m)
    private final DoubleBuffer elevationBuffer = new DoubleBuffer(Constant.ELEVATION_SMOOTHING_FACTOR);

    // A buffer of the recent distance readings (m) for calculating grade
    private final DoubleBuffer distanceBuffer = new DoubleBuffer(Constant.DISTANCE_SMOOTHING_FACTOR);

    // A buffer of the recent grade calculations (%)
    private final DoubleBuffer gradeBuffer = new DoubleBuffer(Constant.GRADE_SMOOTHING_FACTOR);
    ...
    public void addLocation(Location location, int minRecordingDistance) {
        if (!LocationUtils.isValidLocation(location)) {
            ...
            if (location.getLatitude() == Constant.PAUSE_LATITUDE) {
        ...
        if (movingDistance < minRecordingDistance && location.getSpeed() < Constant.MAX_NO_MOVEMENT_SPEED) {
            updateTime(location.getTime());
            lastLocation = location;
            return;
        }
    ...
    void updateSpeed(long time, double speed, long lastLocationTime, double lastLocationSpeed) {
        if (!isValidSpeed(time, speed, lastLocationTime, lastLocationSpeed)) {
            Log.d(Constant.MODULE_TAG, "Invalid speed. speed: " + speed + " lastLocationSpeed: " + lastLocationSpeed);
            return;
        }
    ...
    private boolean isValidSpeed(long time, double speed, long lastLocationTime, double lastLocationSpeed) {
        ...
        if (speedDifference > Constant.MAX_ACCELERATION * timeDifference) {
            return false;
        }
        ...
        return (speed < average * 10) && (diff < Constant.MAX_ACCELERATION * timeDifference);
    }
}
